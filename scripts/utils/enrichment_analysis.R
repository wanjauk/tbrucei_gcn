...
output_module_enrichment_results <- function(results, module_output_dir, 
    ...
    for (mod_color in names(results)) {
        ...
        # write output
        if (nrow(output_df) > 0) {
            filename <- sprintf("%s_%s_enrichment.csv", mod_color, 
                                enrichment_type)
            write.csv(output_df,
    ...
    for (result_name in names(results)) {
        ...
        if (nrow(over_rep) > 0) {
           ...
           # if (!sum(out$num_in_subset) == 0){
                out <- out %>% dplyr::rename(adj_pval=over_represented_pvalue_adj,
                                             term=term.y,
                                             ontology=ontology.y)
                
                openxlsx::write.xlsx(out,
                                 file = file.path(output_dir,"top_go_over-rep_terms_adj_pvalue_sorted.xlsx"))
           # }
        ...
        if (nrow(under_rep) > 0) {
           ...
           # if (!sum(out$num_in_subset) == 0){
                ...
                out <- out %>% dplyr::rename(adj_pval=under_represented_pvalue_adj,
                                             term=term.y,
                                             ontology=ontology.y)
                
                openxlsx::write.xlsx(out,
                                 file = file.path(output_dir, "top_go_under-rep_terms_adj_pvalue_sorted.xlsx"))
           # }
