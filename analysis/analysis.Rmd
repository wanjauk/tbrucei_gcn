...
dev.off()
```
```{r fig.cap="Normalized sample correlation heatmap"}
...
dev.off()
```
```{r fig.cap="Normalized sample density plot"}
...
dev.off()
```
```{r fig.cap="Normalized sample heatmap"}
...
dev.off()
```
```{r fig.cap="Normalized sample boxplot"}
...
# TODO: Add columns such as logFC, FDR e.t.c from topTags to the above object and rename first column. write out csv file of upregulated and downregulated genes and create volcano plot and MA plot to interpret results.
```
\newpage
Plotting to visually interpret differential gene expression results.
...
```
\newpage
```{r, fig.cap="Differentially expressed genes in the proventriculus compared to midgut"}
...
```
\newpage
```{r, fig.cap="Differentially expressed genes in the salivary gland compared to proventriculus"}
